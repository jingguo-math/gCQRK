...
% Problem parameters
alp_values = [0.3 0.4 0.5 0.6 0.7]; % Vector of alp values to test
bet = 0.2;      % Exponent for the test function f(t) = t^bet
Nvec = 2*2.^[2:9]; % Vector of discretization points (N values)

% Test function definition f(t) = t^bet
f = @(t) t.^bet;

% Initialize cell array to store errors for each alp value
all_errors = cell(length(alp_values), 1);

% Main convergence study loop over different alp values
for alp_idx = 1:length(alp_values)
    alp = alp_values(alp_idx);
    
    % Kernel function K(z) = (z+I)^(-alp) (discrete version)
...
% Define line styles and marker types for different alp values
