...
bet_values = [-0.1, 0.3, 0.7, 1.1, 1.5]; % Test different regularity exponents β
Nvec = 4 * 2.^(1:8);    % Vector of grid sizes (N = [8, 16, 32, ..., 1024])

% Kernel function for fractional integral: K(z) = z^(-α)
Kfun = @(z) z.^(-alp);

%% Initialize storage for errors
all_errors = cell(length(bet_values), 1); % Cell array to store errors for each β
...
for bet_idx = 1:length(bet_values)
    ...
    % Formula: grad = max(1, p/(α+β)) where p is the scheme order
    grad = max(1, Ord/(alp + bet));
    
    %% Define Exact Solution and Right-Hand Side
    % Right-hand side function: f(t) = t^β
    f = @(t) t.^bet;
    
    % Exact solution of fractional integral: I^α t^β = Γ(β+1)/Γ(α+β+1) t^{α+β}
